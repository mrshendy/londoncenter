            ...
            if (id_country) {
                $.ajax({
                    ...
                    success: function(data) {
                        ...
                        $('select[name="id_area"]').append(
                            '<option value="" selected disabled>{{ trans('client_management_trans.select_option') }}</option>'
                        );
                        $('select[name="id_city"]').append(
                            '<option value="" selected disabled>{{ trans('client_management_trans.select_option') }}</option>'
                        );
                        $('select[name="id_governmentes"]').append(
                            '<option value="" selected disabled>{{ trans('client_management_trans.select_option') }}</option>'
                        );
        ...
        if (id_governmentes) {
            $.ajax({
                ...
                success: function(data) {
                    ...
                    $.each(data, function(key, value) {
                        $('select[name="id_city"]').append('<option value="' + key + '">' + value +
                            '</option>');
                    });
                },
            });
