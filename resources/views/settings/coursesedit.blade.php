                        ...
                        <ul class="nav nav-pills nav-customs nav-danger mb-3" role="tablist">
                            <li class="nav-item">
                                <a class="nav-link active" data-bs-toggle="tab" href="#border-navs-home"
                                    role="tab">{{ trans('settings_trans.courses_data') }}</a>
                            </li>
                            <li class="nav-item">
                                <a class="nav-link" data-bs-toggle="tab" href="#border-navs-profile"
                                    role="tab">{{ trans('settings_trans.courses_ditils') }}</a>
                            </li>
                        </ul><!-- Tab panes -->
                        <div class="tab-content text-muted">
                            <div class="tab-pane active" id="border-navs-home" role="tabpanel">
                                <form action="{{ route('courses_settings.update', 'test') }}" method="post"
                                    ...
                                    <div class="row">
                                        <div class="form-group">
                                            ...
                                            <select name="status" id="statusf" class="form-control" required>
                                                <option value="1" @if ($courses->status == 1) selected @endif>
                                                    {{ trans('settings_trans.default_true') }}</option>
                                                <option value="0" @if ($courses->status == 0) selected @endif>
                                        ...
                                        <div class="col-4 mb-3">
                                            ...
                                            <input type="text" class="form-control" id="name_en" name="name_en"
                                                value="{{ $courses->getTranslation('name', 'en') }}"
                                        ...
                                        <div class="col-4 mb-3">
                                            <label class="my-1 mr-2"
                                                for="inlineFormCustomSelectPref">{{ trans('settings_trans.sections_select') }}</label>
                                            <select name="sections" class="form-control SlectBox">
                                                <!--placeholder-->
                                                <option value="" selected disabled>
                                                    {{ trans('settings_trans.sections_select') }}
                                                </option>
                                                @foreach ($sections as $data)
                                                    <option value="{{ $data->id }}"
                                                        @if ($data->id == $courses->id_sections) selected @endif>
                                                        {{ $data->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                        <div class="col-6 mb-3">
                                            ...
                                            <textarea class="form-control" id="introduction_ar" name="introduction_ar" rows="4">{{ $courses->getTranslation('introduction', 'ar') }}</textarea>
                                        </div>
                                        <div class="col-6 mb-3">
                                            <label for="name_en"
                                                class="form-label">{{ trans('settings_trans.course_content_ar') }}</label>
                                            <textarea class="form-control" id="course_content_ar" name="course_content_ar" rows="4">{{ $courses->getTranslation('course_content', 'ar') }}</textarea>
                                        </div>
                                        <div class="col-6 mb-3">
                                            <label for="name_en"
                                                class="form-label">{{ trans('settings_trans.introduction_en') }}</label>
                                            <textarea class="form-control" id="introduction_en" name="introduction_en" rows="4">{{ $courses->getTranslation('introduction', 'en') }}</textarea>
                                        </div>
                                        <div class="col-6 mb-3">
                                            <label for="name_en"
                                                class="form-label">{{ trans('settings_trans.course_content_en') }}</label>
                                            <textarea class="form-control" id="course_content_en" name="course_content_en" rows="4">{{ $courses->getTranslation('course_content', 'en') }}</textarea>
                                        </div>
                                        <div class="text-end">
                                            <button type="submit"
                            ...
                            <div class="tab-pane" id="border-navs-profile" role="tabpanel">
                                ...
                                <table id="example"
                                    ...
                                    <tbody>
                                        <?php $i = 0; ?>
                                        @foreach ($courses_details as $data)
                                            <tr>
                                                <?php $i++; ?>
                                                <td>{{ $i }}</td>
                                                <td>{{ $data->id }}</td>
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <div class="modal-content border-0 overflow-hidden">
                                                        ...
                                                        <div class="modal-body">
                                                            <form
                                                                action="{{ route('courses_details_settings.update', 'test') }}"
                                                    ...
                                                    <div class="modal-content border-0 overflow-hidden">
                                                        ...
                                                        <div class="modal-body text-center p-5">
                                                            ...
                                                            <div class="mt-4 pt-4">
                                                                ...
                                                                <form
                                                                    action="{{ route('courses_details_settings.destroy', 'test') }}"
                                                                    method="post">
                                                                    {{ method_field('delete') }}
                                                                    {{ csrf_field() }}
                                                                    <input class="form-control" id="id"
            ...
            <div id="signupModal" class="modal fade" tabindex="-1" aria-hidden="true" style="display: none;">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content border-0 overflow-hidden">
                        ...
                        <div class="modal-body">
                            <form action="{{ route('courses_details_settings.store') }}" method="post"
                                enctype="multipart/form-data">
                                {{ csrf_field() }}
                                <div class="mb-3">
